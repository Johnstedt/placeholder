...
THIS IS THE JOHNSON and FLOYD, unoptimized, the model is a wierd attempt to make it a game theoretic strategy,
ignore that.

The plot_fee_curve function, builds the three dim image in my thesis report.
Much of the array shit is to get every dot to draw the three dim fig, so much may be ignored.

First step may be to create a JOHNSON or FLOYD WARSHALL THAT WORKS NAIVLY, IMPROVE IT TO MAX. Before finding
greedy alternatives.
...
def create_price_model():
	...
	for i in range(10):
		...
		model = {
			"base_price": list(range(10000, 1350000, 10000)),
